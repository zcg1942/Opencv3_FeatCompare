...
void ImageRotationTransformation::transform(float t, const cv::Mat& source, cv::Mat& result) const
{
    ...
    cv::warpAffine(source, result, rotationMat, source.size(), cv::INTER_CUBIC);//旋转变换
}
...
void ImageScalingTransformation::transform(float t, const cv::Mat& source, cv::Mat& result)const
{
    ...
    cv::resize(source, result, dstSize, cv::INTER_AREA);//尺度变换
}
...
void GaussianBlurTransform::transform(float t, const cv::Mat& source, cv::Mat& result)const
{
    ...
    cv::GaussianBlur(source, result, cv::Size(kernelSize,kernelSize), 0);//高斯模糊
}
...
{
    result = source + cv::Scalar(t,t,t,t);//亮度变换
}

#pragma mark - CombinedTransform implementation//综合变换 将两种变换结合起来
